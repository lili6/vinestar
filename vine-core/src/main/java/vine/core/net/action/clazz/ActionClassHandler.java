...
public class ActionClassHandler extends ActionHandler {
    ...
    public void executeAction(Object[] params,
        ...
        Integer msgId = null;
        HttpPacket resp = null;
        for (Object param : params) {
            if (param instanceof HttpPacket) {
                Integer tmpMsgId = ((HttpPacket) param).getPacketId();
                resp =(HttpPacket) param;
...
//		if (log.isDebugEnabled()){log.debug("before method invoke params===={}\n",params);}
        HttpPacket result = null;
        try {// 运行该Action处理
            Object ret = method.invoke(action, params);
            if (null != ret && ret instanceof HttpPacket) {
                result = (HttpPacket) ret;
