...
public abstract class UserSession implements Serializable {
	...
	/** 上次实际发送给客户端的消息 */
	protected transient HttpPacket lastResponseMessage = null;
	/** 缓存中等待的消息 */
	protected transient List<HttpPacket> waitMessage = Collections.synchronizedList(new ArrayList<HttpPacket>());
	protected ConcurrentLinkedQueue<HttpPacket> waitMessageQueue = new ConcurrentLinkedQueue<HttpPacket>();
	...
	/** 用户会话创建监听器 */
	protected List<UserSessionCreatedListener> createdListeners = new ArrayList<UserSessionCreatedListener>();
	/** 接收到消息的时间 */
	...
	public void lock(int packetId) {
		if (requestLocker == null) requestLocker = new ConcurrentSkipListSet<Integer>();
	 ...
	 */
	protected abstract void responseImmediately(HttpPacket packet);
	
	/**
	 * 向客户端响应消息，缓冲处理
	 * @param packet 要发送的消息对象
	 */
	public abstract void response(HttpPacket packet);
    ...
    */
	 ...
	 * @param packet
	 */
	public abstract void push(HttpPacket packet);
	/**
	 * 消息发送前处理
	 * @param packet
	 */
	protected void beforeResponse(HttpPacket packet) {
...
//		if (log.isDebugEnabled()) { log.debug("beforeResponse响应数据过滤后[{}]", packet); }
	 ...
	 * @return
	 */
	public HttpPacket popWaitMessage() {
		if (waitMessage == null) return null;
		int size = waitMessage.size();
		if (size == 0) return null;
        HttpPacket data = null;
		if(size == 1){
			data = waitMessage.get(0);
			waitMessage.clear();
			return data;
		}
		List<HttpPacket> dataList = new ArrayList<HttpPacket>();
		for (HttpPacket data2 : waitMessage) {
			dataList.add(data2);
		}
		waitMessage.clear();
		return data;
	 ...
	 * @return
	 */
	public  List<HttpPacket> getWaitMessage() {
		return waitMessage;
	}
	 ...
	 * @return
	 */
	public HttpPacket getLastResponseMessage() {
